...
We have verified our method along with four other related works to answer this question. All the above mentioned datasets were randomly divided into three sets - training (70\%), validation (15\%) and test (15\%). Prior researchers who worked with these datasets have used \textit{Logistic Regression} as classification model \cite{Kamishima,NIPS2017_6988,Hardt}. We also decided to use this learner. Before moving to results, here we briefly describe prior works which we selected to for our study. Prior work in fairness domain can be classified into three categories -
...
\centering
\tiny
\caption{Results showing the change of Recall and False alarm before and after bias mitigation algorithm applied. The gray cells indicate improvement and black cells indicate damage.}
...
Table \ref{tbl:fairness_cost} shows the results of our approach (FLASH) and four other algorithms developed by previous researchers. Two goals were considered here - recall and false alarm. The first one is higher the better and the second one is lower the better. We can see that there are very few gray cells and a lot of black cells indicating that achieving fairness damages recall and false alarm - which bolsters the conclusion made by Berk et al.\cite{berk2017convex}. 
...
\tiny
\centering
\caption{Results showing the change of four goals before and after hyperparameter optimization. The gray cells show improvement and black cells show damage}
...
So, we are making fairness mitigation problem a multiobjective optimization problem. When no. of goals increase, optimization becomes costly. So, we decided to ask the third research question - 
